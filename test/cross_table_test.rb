...
class CrossTableTest < Test::Unit::TestCase
  ...
  def test_group_count
    ...
    mins = CrossTable.from(
        recs: JOBS, group_rules: [prices, oses],
        y_keys: prices.keys, x_keys: oses.keys, ret_type: :data_only) do |group_recs|
    ...
    levels = {
      ...
      easy:   ->(rec){:easy == rec[:level]},
      normal: ->(rec){:normal == rec[:level]},
      hard:   ->(rec){:hard == rec[:level]},
    }
  
    level_counts = CrossTable.avgs_from(
